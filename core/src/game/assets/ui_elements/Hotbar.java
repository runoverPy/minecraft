...
public class Hotbar extends FlexBox {
    ...
    private void checkSlot(int slot) throws IndexOutOfBoundsException {
        if (0 > slot || slot > 8) throw new IndexOutOfBoundsException("Invalid value " + slot + ": only values between 0 and 8 allowed");
    }

    public void draw(Matrix4f matrix4f) {
        glDisable(GL_DEPTH_TEST);
        int pxScale = getPixelScale() / 2;
