...
import game.assets.ui_elements.ContentsRoot;
...
@Deprecated
public abstract class UIELMenu extends ContentsRoot implements Menu {
            ...
            for (List<Widget> row : widgets) {
                if (row.size() != 0) {
                    ...
                    for (Widget widget : row) {
                        UIELMenu.this.insert(widget.create(width, height, runningX, runningY, UIELMenu.this));
    ...
    protected TableOrganizer tableOrganizer(int nCols, int nRows, int rowSize, int colSize, int padding) {
    ...
    protected class TableOrganizer {
        ...
        public void insert(Widget widget, int startCol, int numCols, int startRow, int numRows) {
            ...
            UIELMenu.this.insert(widget.create(width, height, xOffset, yOffset, UIELMenu.this));
        }
    }
